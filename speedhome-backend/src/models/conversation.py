...
class Conversation(db.Model):
    ...
    # Booking context - this is what enables our "Rules of Engagement"
    booking_id = db.Column(db.Integer, db.ForeignKey('bookings.id'), nullable=False)
    property_id = db.Column(db.Integer, db.ForeignKey('properties.id'), nullable=False)
    
    # Conversation status and metadata
    status = db.Column(db.String(20), default='active')  # 'active', 'read_only', 'closed'
    ...
    # Unique constraint to prevent duplicate conversations for the same booking
    __table_args__ = (db.UniqueConstraint('tenant_id', 'landlord_id', 'booking_id', name='unique_conversation_per_booking'),)
    ...
    def can_send_message(self, user_id, booking_status):
        """
        Implement the "Rules of Engagement" logic
        Returns True if the user can send a message in this conversation
        """
        ...
        # Check booking status (our main rule)
        if booking_status in ['pending', 'confirmed']:
            return True
        
        # Booking is cancelled, declined, or completed - make read-only
        if booking_status in ['cancelled', 'declined', 'completed']:
            # Update conversation status to read-only
